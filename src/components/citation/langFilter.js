...
const Radio = ({name, onChange, lfState}) => {
  ...
  const Label = styled.label`
    color: ${props => (props.isSelected ? "hsla(150,100%,10%,0.9)" : "hsla(150,100%,30%,0.9)")};;
    background-color : ${props => (props.isSelected ? "white" : "mintcream")};
    border: 1px solid ${props => (props.isSelected ? "hsla(150,100%,40%,0.9)" : "white")};
    border-radius: 4px;
    padding: 2px 4px;
    margin: 0px 2px;
  `;
...
const LangFilter = ({names, handleRadioLang, lf}) => {
    return(
      <div id="lf">
        ...
        :
        {names.map((name) => {
          return <Radio name={name} onChange={handleRadioLang} lfState={lf} key={name}/>
        })}
      </div>
    )
  }
